module.exports = {
  ...
  formatDate: function (originalData, fmt) {
    if (originalData && (isNaN(originalData) || (originalData instanceof Date))) {
      ...
      let date = originalData instanceof Date ? originalData : new Date(originalData)
  ...
  computerAge: function (birthDay) {
    let t = new Date().getTime(), b = this.notNull(birthDay) ? new Date(birthDay) : null, age = '未知'
  ...
  initScrollbar: function (selector, userConfig) {
    let target = $(selector), config = {}, baseConfig = {
      axis: 'yx',
      setWidth: '100%',
      setTop: '0px',
      theme: 'dark-thin',
  ...
  maskMobile: function (number) {
    if (number.length == 11) {
      let front = number.substring(0, 3), last = number.substring(7)
      number = front + ' * * * * ' + last
    }
  ...
  delayLoad: function (target, callback, delay_time, retry) {
    ...
    let load_obj = new load(target, callback, delay_time, retry)
    load_obj._delayLoad()
  },
